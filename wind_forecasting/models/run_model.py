...
if __name__ == "__main__":
    ...
    try:
        if local_rank == 0:
            print(f"\n{Colors.BOLD_BLUE}Processing {data_module.dataset.__clase__.__name__} dataset...{Colors.ENDC}")
        
        ## TRAIN
        train_results = model.train(data_module=data_module) # TODO JUAN WHY TEST IN RANK 0?
        
        if local_rank == 0:

            ## TEST
            test_results = model.test(data_module=data_module)
