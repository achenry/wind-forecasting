...
#SBATCH --gres=gpu:4
#SBATCH --time=1-00:00
#SBATCH --job-name=smoothed60_compare
#SBATCH --output=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/smoothed60_compare_%j.out
#SBATCH --error=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/smoothed60_compare_%j.err
#SBATCH --hint=nomultithread
#SBATCH --distribution=block:block
#SBATCH --gres-flags=enforce-binding

# =============================================================================
# QUICK COMPARISON TUNING SCRIPT
# Purpose: Run 10-epoch, 20-trial experiments to compare normalization strategies
# Usage: sbatch tune_storm_smoothed_awaken_p60_compare.sh <model> <norm_type>
#        model: informer, autoformer, spacetimeformer
#        norm_type: external, internal
# Example: sbatch tune_storm_smoothed_awaken_p60_compare.sh informer external
# =============================================================================

# --- Arguments ---
export MODEL_NAME="${1:-informer}"
export NORM_TYPE="${2:-external}"
...
# --- Config Selection Based on Normalization Type ---
if [ "$NORM_TYPE" == "external" ]; then
    export CONFIG_FILE="${BASE_DIR}/config/training/training_inputs_storm_awaken_smoothed_pred60_external.yaml"
else
    export CONFIG_FILE="${BASE_DIR}/config/training/training_inputs_storm_awaken_smoothed_pred60_internal.yaml"
fi

export RESTART_TUNING_FLAG="--restart_tuning"  # Always start fresh for comparison
...
# --- Print Job Info ---
echo "=============================================="
echo "QUICK COMPARISON TUNING - SMOOTHED AWAKEN"
echo "=============================================="
...
echo "Launching ${NUM_GPUS} tuning workers for QUICK COMPARISON..."
...
for i in $(seq 0 $((${NUM_GPUS}-1))); do
    ...
    WORKER_RANK=${i} CUDA_VISIBLE_DEVICES=${i} nohup bash -c "
        ...
        echo \"Worker ${i}: Running QUICK COMPARISON (10 epochs, 20 trials)...\"

        # OVERRIDE for quick comparison: max_epochs=10, total_study_trials=20
        python ${WORK_DIR}/run_scripts/run_model.py \\
          ...
          ${RESTART_TUNING_FLAG} \\
          --single_gpu \\
          --override optuna.max_epochs=10 optuna.total_study_trials=20 optuna.n_trials_per_worker=5
