...
#SBATCH --gres=gpu:4
#SBATCH --time=1-00:00
#SBATCH --job-name=smoothed60_tune_ext_allgpu
#SBATCH --output=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/smoothed60_tune_ext_allgpu_%j.out
#SBATCH --error=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/smoothed60_tune_ext_allgpu_%j.err
#SBATCH --hint=nomultithread
#SBATCH --distribution=block:block
#SBATCH --gres-flags=enforce-binding

# =============================================================================
# FULL TUNING SCRIPT - EXTERNAL NORMALIZATION (ALL_GPU PARTITION)
# Purpose: Hyperparameter tuning with flexible node access (cfdg + mpcg)
# Partition: all_gpu.p (1-day max, can access A100 and H100 GPUs)
# Usage: sbatch tune_storm_smoothed_awaken_p60_external_allgpu.sh <model> [--restart_tuning]
# =============================================================================
...
# --- Print Job Info ---
echo "=============================================="
echo "TUNING - EXTERNAL NORMALIZATION (ALL_GPU)"
echo "=============================================="
...
echo "Launching ${NUM_GPUS} tuning workers (24h session)..."

for i in $(seq 0 $((${NUM_GPUS}-1))); do
    # Use different seed offsets for allgpu jobs (2000+ series)
    CURRENT_WORKER_SEED=$((2042 + i*100))
    ...
    WORKER_RANK=${i} CUDA_VISIBLE_DEVICES=${i} nohup bash -c "
        ...
        echo \"Worker ${i}: Running TUNING (config defaults)...\"
...
echo "=============================================="
echo "TUNING COMPLETED - EXTERNAL NORMALIZATION (ALL_GPU)"
echo "Model: ${MODEL_NAME}"
