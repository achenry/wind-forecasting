...
#SBATCH --gres=gpu:4
#SBATCH --time=7-00:00
#SBATCH --job-name=tactis_tune_p2
#SBATCH --output=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/tactis_tune_p2_%j.out
#SBATCH --error=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/tactis_tune_p2_%j.err
#SBATCH --hint=nomultithread
#SBATCH --distribution=block:block
#SBATCH --gres-flags=enforce-binding

# =============================================================================
# TACTIS-2 PHASE 2 TUNING - COPULA PATH
# Purpose: Phase 2 of two-stage TACTiS-2 tuning (freeze flow, train copula)
# Usage: sbatch tune_tactis_phase2_storm_smoothed_p60.sh [--restart_tuning]
# Example: sbatch tune_tactis_phase2_storm_smoothed_p60.sh
#          sbatch tune_tactis_phase2_storm_smoothed_p60.sh --restart_tuning
# =============================================================================
...
# --- Print Job Info ---
echo "=============================================="
echo "TACTIS-2 PHASE 2 TUNING - COPULA PATH"
echo "=============================================="
...
echo "=== STARTING PARALLEL OPTUNA TUNING WORKERS (PHASE 2) ==="
date +"%Y-%m-%d %H:%M:%S"
...
echo "Launching ${NUM_GPUS} tuning workers for PHASE 2 TUNING..."
...
for i in $(seq 0 $((${NUM_GPUS}-1))); do
    ...
    WORKER_RANK=${i} CUDA_VISIBLE_DEVICES=${i} nohup bash -c "
        ...
        echo \"Worker ${i}: Running PHASE 2 TUNING (copula path)...\"
...
echo "=============================================="
echo "PHASE 2 TUNING COMPLETED - TACTIS-2"
echo "Model: ${MODEL_NAME}"
