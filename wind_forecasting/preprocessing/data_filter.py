        ...
        if parallel == "feature":
            ...
            with executor as ex:
                ...
                for k, v in futures.items():
                    ...
                    if imputed_vals is not None:
                        df.update(imputed_vals, on="time").with_columns(cs.starts_with(k).cast(dtype)).collect().write_parquet(save_path, statistics=False)
            ...
            for feat_type in impute_missing_features:
                ...
                imputed_vals = imputing.impute_all_assets_by_correlation(
                    ...
                    data_pl=df.select(features_pl), data_pd=None,
                                                            ...
                                                            save_path=save_path)
                if imputed_vals is not None:
                    df.update(imputed_vals, on="time").with_columns(cs.starts_with(feat_type).cast(dtype)).collect().write_parquet(save_path, statistics=False)
                    df = pl.scan_parquet(save_path)
            ...
            for feat_type in impute_missing_features:
                ...
                imputed_vals = imputing.impute_all_assets_by_correlation(
                                                            ...
                                                            save_path=save_path)
                if imputed_vals is not None:
                    df.update(imputed_vals, on="time").with_columns(cs.starts_with(feat_type).cast(dtype)).collect().write_parquet(save_path)
                    df = pl.scan_parquet(save_path)
                # df_cols.append(imputed_vals.select(cs.starts_with(f"{feat_type}_")))
                logging.info(f"Imputed feature {feat_type} in DataFrame {df_idx}.")
        return df
