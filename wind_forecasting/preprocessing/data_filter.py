...
class DataFilter:
    ...
    def filter_inoperational(
        self, df, status_codes=None, availability_codes=None, include_nan=True
    ) -> pl.LazyFrame:
        """
        ...
        if self.turbine_status_col and not any(
            self.turbine_status_col in col for col in cols
        ):
            logging.info(
                f"Status column '{self.turbine_status_col}' not found in data, skipping status filtering"
            )
            status_codes = None

        if self.turbine_availability_col and not any(
            self.turbine_availability_col in col for col in cols
        ):
            logging.info(
                f"Availability column '{self.turbine_availability_col}' not found in data, skipping availability filtering"
            )
        ...
        if status_codes is None and availability_codes is None:
            logging.info(
                "No status or availability columns found, skipping inoperational filtering"
            )
            return df

        # Create masks for filtering based on data format
        if self.data_format == "wide":
            return self._filter_inoperational_wide(
                df, status_codes, availability_codes, include_nan
            )
        else:
            return self._filter_inoperational_long(
                df, status_codes, availability_codes, include_nan
            )

    def _filter_inoperational_wide(
        self, df, status_codes, availability_codes, include_nan
    ):
        include_status_mask = (
            status_codes is not None and self.turbine_status_col is not None
        )
        include_availability_mask = (
            availability_codes is not None and self.turbine_availability_col is not None
        )

        if include_status_mask and include_availability_mask:
            return df.with_columns(
                pl.when(
                    cs.starts_with(self.turbine_status_col).is_in(status_codes)
                    | (
                        cs.starts_with(self.turbine_status_col).is_null()
                        if include_nan
                        else False
                    )
                ).then(cs.starts_with(self.turbine_status_col)),
                pl.when(
                    cs.starts_with(self.turbine_availability_col).is_in(
                        availability_codes
                    )
                    | (
                        cs.starts_with(self.turbine_availability_col).is_null()
                        if include_nan
                        else False
                    )
    ...
    def _filter_inoperational_long(
        self, df, status_codes, availability_codes, include_nan
    ):
        include_status_mask = (
            status_codes is not None and self.turbine_status_col is not None
        ...
        if include_status_mask and include_availability_mask:
            return df.filter(
                (
                    ...
                    | (
                        pl.col(self.turbine_status_col).is_null()
                        if include_nan
                        else False
                    )
                )
                & (
                    ...
                    | (
                        pl.col(self.turbine_availability_col).is_null()
                        if include_nan
                        else False
                    )
                )
    ...
    def _single_generate_window_range_filter(self, df_query, tid, **kwargs):
        mask = filters.window_range_flag(
            window_col=df_query.select(f"wind_speed_{tid}")
            .collect()
            .to_pandas()[f"wind_speed_{tid}"],
            value_col=df_query.select(f"power_output_{tid}")
            .collect()
            .to_pandas()[f"power_output_{tid}"],
            **kwargs,
        ).values
        mask &= (
            df_query.select(pl.all_horizontal(pl.all().is_not_null()))
            .collect()
            .to_numpy()
            .flatten()
        )
        ...
        logging.info(
            f"Finished generating out of window filter for {df_query.collect_schema().names()}"
        )
        return mask

    def _single_generate_bin_filter(self, df_query, tid, **kwargs):
        mask = filters.bin_filter(
        ...
        mask = (
            mask.to_numpy().flatten().astype(bool)
            & df_query.select(pl.all_horizontal(pl.all().is_not_null()))
            .collect()
            .to_numpy()
            .flatten()
        ...
        logging.info(
            f"Finished generating wind speed-power curve bin-outlier filter for {df_query.collect_schema().names()}"
        )
        return mask  # , center

    def _single_generate_std_range_filter(self, df_query, tid, **kwargs):
        mask = filters.std_range_flag(
            data_pl=df_query.collect().to_pandas(), **kwargs
        ).values
        mask &= df_query.select(pl.all().is_not_null()).collect().to_numpy()

        logging.info(
            f"Finished generating std out of range filter for {df_query.collect_schema().names()}"
        )
        return mask
    ...
    def smooth(
        ...
        feature_types,
        dtype,
        smoothing_function="butterworth",
        smoothing_params=None,
        plot=False,
    ):

        sub_df = df_query.select(
            [cs.starts_with(feat_type) for feat_type in feature_types]
        )
        features = sub_df.collect_schema().names()

        if smoothing_function == "moving_average":
            df_query_filt = df_query.with_columns(
                [
                    cs.starts_with(feat_type).rolling_mean_by(
                        "time", window_size=f"{smoothing_params['window_size']}s"
                    )
            ...
            filter_coeffs_1 = 1 / np.sqrt(
                1
                + (fs_1 / smoothing_params["freq_cutoff"])
                ** (2 * smoothing_params["order"])
            )
            filter_coeffs_2 = 1 / np.sqrt(
                1
                + (fs_2 / smoothing_params["freq_cutoff"])
                ** (2 * smoothing_params["order"])
            )
            ...
            df_query_filt = df_query.with_columns(
                [
                    pl.Series(name=feat, values=ts[:, i]).cast(dtype)
            ...
            df_query_filt = df_query_filt.select(
                pl.all().slice(400, pl.len() - 800)
            )  # remove transient at beginning, TODO figure out how much to remove based on dt and freq_cutoff and order
                ...
                [
                    pl.Series(
                        ...
                        values=savgol_filter(
                            sub_df.collect().to_numpy(),
                            window_length=smoothing_params["window_size"],
                            polyorder=smoothing_params["order"],
                            axis=0,
                        )[:, i],
                    ).cast(dtype)
                    for i, feat in enumerate(features)
        ...
        if plot:
            ...
            for ax_idx, feat_type in enumerate(feature_types):
                ax[ax_idx].plot(
                    df_query.select("time").collect().to_numpy(),
                    df_query.select([f"{feat_type}{sfx}" for sfx in suffixes])
                    .collect()
                    .to_numpy(),
                    linestyle="-",
                    label="original",
                ...
                filt_sig = (
                    df_query_filt.select([f"{feat_type}{sfx}" for sfx in suffixes])
                    .collect()
                    .to_numpy()
                )
            ...
            fig.suptitle(
                f"{' '.join([w.capitalize() for w in smoothing_function.split('_')])}"
            )
            fig.show()

        return df_query_filt

    def multi_generate_filter(
        self, df_query, filter_func, feature_types, turbine_ids, **kwargs
    ):
        if self.multiprocessor:
            ...
            for tid in turbine_ids:
                res = filter_func(
                    df_query=df_query.select(
                        [pl.col(f"{feat_type}_{tid}") for feat_type in feature_types]
                    ),
                    tid=tid,
                    **kwargs,
                )
    ...
    def _single_compute_bias(self, df_query, tid):
        bias = (
            df_query.filter(pl.col(f"power_output_{tid}") >= 0)
            .select(
                "time",
                f"wind_direction_{tid}",
                f"nacelle_direction_{tid}",
                "wd_median",
                "nd_median",
            )
            .select(
                wd_bias=(pl.col(f"wind_direction_{tid}") - pl.col("wd_median")),
                nd_bias=(pl.col(f"nacelle_direction_{tid}") - pl.col("nd_median")),
            )
            .select(
                pl.all().radians().sin().mean().name.suffix("_sin"),
                pl.all().radians().cos().mean().name.suffix("_cos"),
            )
            .select(
                wd_bias=pl.arctan2("wd_bias_sin", "wd_bias_cos").degrees().mod(360),
                nd_bias=pl.arctan2("nd_bias_sin", "nd_bias_cos").degrees().mod(360),
            )
            .select(
                pl.when(pl.all() > 180.0).then(pl.all() - 360.0).otherwise(pl.all())
            )
            .select(
                0.5 * (pl.col("wd_bias").fill_null(0) + pl.col("nd_bias").fill_null(0))
            )
            .collect()
            .item()
        )
        ...
        logging.info(
            f"Finished computing wind/nacelle direction bias for turbine {tid}"
        )
        return bias
    ...
    def fill_multi_missing_datasets(
        self, dfs, impute_missing_features, interpolate_missing_features, r2_threshold
    ):
        if self.multiprocessor:
            # if self.multiprocessor == "mpi" and mpi_exists:
            #     executor = MPICommExecutor(MPI.COMM_WORLD, root=0)
            ...
            with executor as ex:
                futures = [
                    ex.submit(
                        self._fill_single_missing_dataset,
                        df_idx=df_idx,
                        df=df,
                        impute_missing_features=impute_missing_features,
                        interpolate_missing_features=interpolate_missing_features,
                        parallel="turbine_id",
    ...
    def _impute_single_missing_dataset(
        self,
        df_idx,
        df,
        save_path,
        ...
        if parallel == "feature":
            ...
            with executor as ex:
                futures = {
                    feature: ex.submit(
                        ...
                        data_pl=df.select("time", cs.starts_with(f"{feature}_")),
                        data_pd=None,
                        #  data_pd=unpivot_df.select(["time", "turbine_id", feature]).collect().to_pandas().set_index(["time", "turbine_id"]),
                        impute_col=feature,
                        reference_col=feature,
                        asset_id_col="turbine_id",
                        method="linear",
                        r2_threshold=r2_threshold,
                        save_path=save_path,
                    )
                ...
                for k, v in futures.items():
                    ...
                    if imputed_vals is not None:
                        df.update(imputed_vals, on="time").collect().write_parquet(
                            save_path, statistics=False
                        )
            ...
            for feat_type in impute_missing_features:
                ...
                imputed_vals = imputing.impute_all_assets_by_correlation(
                    ...
                    data_pl=df.select(features_pl),
                    data_pd=None,
                    impute_col=feat_type,
                    reference_col=feat_type,
                    asset_id_col="turbine_id",
                    method="linear",
    ...
    def _fill_single_missing_dataset(
        self,
        df_idx,
        df,
        save_path,
        ...
        df = self._impute_single_missing_dataset(
            df_idx,
            df,
            save_path=save_path,
            impute_missing_features=impute_missing_features,
        ...
        df = df.with_columns(
            [
                cs.starts_with(f"{feat}_")
                .interpolate()
                .fill_null(strategy="forward")
                .fill_null(strategy="backward")
                for feat in interpolate_missing_features
            ]
        )
        ...
        if df.select(pl.any_horizontal(pl.all().is_null().sum())).collect().item():
            missing_columns = (
                df.select(
                    col.name
                    for col in df.select(cs.numeric().null_count() > 0).collect()
                    if col.all()
                )
                .collect_schema()
                .names()
            ...
            logging.warning(
                f"Warning, there are still nulls in dataframe {df_idx} in columns {missing_columns}!"
            )
    ...
    def _js_divergence(p, q):
        m = (1.0 / 2.0) * (p + q)
        return (1.0 / 2.0) * __class__._kl_divergence(p, m) + (
            1.0 / 2.0
        ) * __class__._kl_divergence(q, m)
    ...
    def conditional_filter(
        ...
        """
        if self.data_format == "wide":
            return self._conditional_filter_wide(
                df,
                threshold,
                mask,
                file_set_indices,
            ...
            return self._conditional_filter_long(
                df,
                threshold,
                mask,
                file_set_indices,
    ...
    def _conditional_filter_wide(
        ...
        if check_js:
            if file_set_indices is None:
                ...
                for inp_feat, opt_feat in zip(mask_input_features, output_features):
                    js_score = self._compute_js_divergence(
                        train_sample=df.filter(mask(inp_feat))
                        .select(opt_feat)
                        .drop_nulls()
                        .collect()
                        .to_numpy()
                        .flatten(),
                        test_sample=df.select(opt_feat)
                        .drop_nulls()
                        .collect()
                        .to_numpy()
                        .flatten(),
                    ...
                    logging.info(
                        f"JS Score for feature {opt_feat} = {js_score} with filter {filter_type}"
                    )
                    js_scores.append(js_score)

                    if js_score > threshold:
                        df = df.with_columns(
                            pl.when(mask(inp_feat))
                            .then(None)
                            .otherwise(pl.col(opt_feat))
                            .alias(opt_feat)
                        )

                        logging.info(
                            f"Applied filter {filter_type} to feature {opt_feat}."
                        )
            else:
                # js_scores = {}
                for file_set_idx in file_set_indices:
                    ...
                    for inp_feat, opt_feat in zip(mask_input_features, output_features):
                        js_score = self._compute_js_divergence(
                            train_sample=df.filter(
                                (pl.col("file_set_idx") == file_set_idx)
                            )
                            .filter(mask(file_set_idx, inp_feat))
                            .select(opt_feat)
                            .drop_nulls()
                            .collect()
                            .to_numpy()
                        ...
                        if js_score > threshold:
                            ...
                            df = pl.concat(
                                [
                                    df.filter(
                                        ...
                                        pl.when(mask(file_set_idx, inp_feat))
                                        .then(None)
                                        .otherwise(pl.col(opt_feat))
                                        .alias(opt_feat)
                                    )
                            ...
                            logging.info(
                                f"Applied filter {filter_type} to feature {opt_feat} for file set {file_set_idx}."
                            )
            ...
            if file_set_indices is None:
                ...
                df = df.with_columns(
                    **{
                        opt_feat: pl.when(mask(inp_feat))
                        .then(None)
                        .otherwise(pl.col(opt_feat))
                        for inp_feat, opt_feat in zip(
                            mask_input_features, output_features
                        )
                    }
                )
            else:
                df = pl.concat(
                    [
                        df.filter(pl.col("file_set_idx") == file_set_idx).with_columns(
                            **{
                                opt_feat: pl.when(mask(file_set_idx, inp_feat))
                                .then(None)
                                .otherwise(pl.col(opt_feat))
                                for inp_feat, opt_feat in zip(
                                    mask_input_features, output_features
    ...
    def _conditional_filter_long(
        ...
        if check_js:
            if file_set_indices is None:
                ...
                df = df.with_columns(
                    **{
                        opt_feat: pl.when(mask(inp_feat) & (js_score > threshold))
                        .then(None)
                        .otherwise(pl.col(opt_feat))
                        for js_score, inp_feat, opt_feat in zip(
                            js_scores, mask_input_features, output_features
                        )
                    }
                )
    ...
    def circ_mean(x):
        ...
        y = (
            np.degrees(
                np.arctan2(
                    np.nanmean(np.sin(np.radians(x))), np.nanmean(np.cos(np.radians(x)))
                )
            )
            % 360.0
        )
...
def add_df_continuity_columns(df, mask, dt):
    ...
    return (
        ...
        .with_columns(
            dt=pl.when(pl.int_range(0, pl.len()) == 0)
            .then(np.timedelta64(dt, "s"))
            .otherwise(pl.col("dt"))
        )
        .select(
            "time", "dt", cs.starts_with("num_missing"), cs.starts_with("is_missing")
        )
        .with_columns(continuous=pl.col("dt") == np.timedelta64(dt, "s"))
        .with_columns(
            continuous_shifted=pl.col("continuous").shift(-1, fill_value=True)
        )
    )
...
def add_df_agg_continuity_columns(df):
    ...
    df = df.filter(
        pl.col("continuous") | (~pl.col("continuous") & pl.col("continuous_shifted"))
    )
    start_time_cond = (pl.col("continuous") & ~pl.col("continuous_shifted")).shift() | (
    ...
    return (
        pl.concat(
            [
                df.filter(start_time_cond).select(start_time=pl.col("time")),
                df.with_columns(end_time=pl.col("time").shift(1))
                .filter(end_time_cond)
                .select("end_time"),
            ],
            how="horizontal",
        )
...
def get_continuity_group_index(
    continuity_groups_df, time_series_df, cg_init_idx=0, chunk_size=32
):
    ...
    time_series_df = time_series_df.with_columns(
        continuity_group=pl.when(True).then(pl.lit(-1))
    )
    for i, (start, end) in enumerate(
        continuity_groups_df.select("start_time", "end_time").iter_rows()
    ):
        # print(i, start, end, duration)
        logging.info(
            f"Getting continuity group index {cg_init_idx + i} of {cg_init_idx + total_rows} for start time {start}, end time {end}, duration {end - start}"
        )
        ...
        if (i % chunk_size == 0) or (i == total_rows - 1):
            time_series_df = time_series_df.with_columns(
                continuity_group=group_number.otherwise(pl.col("continuity_group"))
            )
            group_number = None
...
def merge_adjacent_periods(agg_df, dt):
    ...
    for end_time_idx in range(num_periods):
        ...
        if not (end_time_idx + 1 == num_periods) and (
            end_time + timedelta(seconds=dt)
            == all_times.item(end_time_idx + 1, "start_time")
        ):
            continue
    ...
    return pl.DataFrame(
        ...
        schema={
            "start_time": pl.Datetime(
                time_unit=agg_df.collect_schema()["start_time"].time_unit
            ),
            "end_time": pl.Datetime(
                time_unit=agg_df.collect_schema()["end_time"].time_unit
            ),
        },
...
def gauss_corr(gauss_params, power_ratio):
    ...
    gauss = (
        -1
        * gauss_params[2]
        * np.exp(-0.5 * ((xs - gauss_params[0]) / gauss_params[1]) ** 2)
        + 1.0
    )
...
def compute_offsets(
    ...
    turbine_ids,
    turbine_pairs: list[tuple[int, int]] = None,
    plot=False,
    save_path=None,
):
    p_min = 100
    ...
    for pair in prat_turbine_pairs:
        ...
        dir_align = (
            np.degrees(
                np.arctan2(
                    fi.layout_x[i_up] - fi.layout_x[i_down],
                    fi.layout_y[i_up] - fi.layout_y[i_down],
                )
            )
            % 360
        )
        ...
        df_sub = df.filter(
            (pl.col(f"power_output_{tid_up}") >= p_min)
            & (pl.col(f"power_output_{tid_up}") <= p_max)
            & (pl.col(f"power_output_{tid_down}") >= 0)
        ).select(
            f"power_output_{tid_up}",
            f"power_output_{tid_down}",
            f"wind_direction_{tid_up}",
            f"wind_direction_{tid_down}",
        )
        ...
        df_sub = (
            df_sub.with_columns(
                pl.when((pl.col(f"wind_direction_{tid_up}") >= 359.5))
        ...
        df_sub = df_sub.select(
            pl.col(f"wd_round"),
            (
                pl.col(f"power_output_{tid_down}") / pl.col(f"power_output_{tid_up}")
            ).alias("p_ratio"),
        ).collect()

        if (type(plot) is bool and plot) or (
            type(plot) is list and tuple(pair) in plot
        ):
            fig, ax = plt.subplots(1, 1, figsize=(10, 6))
            ax.plot(
                df_sub.select("wd_round").to_numpy().flatten(),
                df_sub.select("p_ratio").to_numpy().flatten(),
                label="_nolegend_",
            )
            ax.plot(
                dir_align * np.ones(2), [0, 1.25], "k--", label="Direction of Alignment"
            )
            ax.grid()
        ...
        wd_range = (
            np.arange(
                ...
                int(np.round(dir_align)) + prat_hfwdth + 1,
            )
            % 360
        )
        ...
        nadir = (
            df_sub.filter(pl.col("wd_round").is_in(wd_range))
            .filter(pl.col("p_ratio") == pl.col("p_ratio").min())
            .select(pl.col("wd_round"))
            .item()
        )
        ...
        if len(
            set(wd_range) - set(df_sub.select(f"wd_round").to_numpy().flatten())
        ):  # > int(prat_hfwdth / 10):
            logging.info(
                f"Cannot compute nadir for turbine pair {turbine_ids[i_up]}, {turbine_ids[i_down]}"
            )
            continue
        ...
        xs = np.arange(
            -int((2 * prat_hfwdth - 1) / 2), int((2 * prat_hfwdth + 1) / 2), 1
        )
        gauss = (
            -1
            * opt_gauss_params.x[2]
            * np.exp(-0.5 * ((xs - opt_gauss_params.x[0]) / opt_gauss_params.x[1]) ** 2)
            + 1.0
        )
        ...
        if (type(plot) is bool and plot) or (
            ...
            ax.plot(
                2 * [nadir + opt_gauss_params.x[0]],
                [0, 1.25],
                "r--",
                label="Direction of Measured Power Nadir",
            )
            ax.legend()
        ...
        print(
            f"Direction offset for turbine pair ({tid_up}, {tid_down}) = {dir_offset}"
        )
