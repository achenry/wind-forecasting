...
from wind_forecasting.preprocessing.data_filter import (
    DataFilter,
    add_df_continuity_columns,
    add_df_agg_continuity_columns,
    get_continuity_group_index,
    group_df_by_continuity,
...
ROW_BOUNDS = (
    datetime(year=2024, month=2, day=20),
    datetime(year=2025, month=3, day=27),
)
...
def main():
    ...
    parser.add_argument("-af", "--apply_filters", action="store_true")
    ...
    RUN_ONCE = (
        (args.multiprocessor != "mpi") or (args.multiprocessor is None)
    )  # or (args.multiprocessor == "mpi" and mpi_exists and (MPI.COMM_WORLD.Get_rank()) == 0)
    ...
    config["raw_data_directory"] = [
        os.path.expanduser(fp) for fp in config["raw_data_directory"]
    ]
    ...
    assert (
        not args.reload_data and os.path.exists(config["processed_data_path"])
    ) or all(os.path.exists(fp) for fp in config["raw_data_directory"]), (
        f"One of paths doesn't exist."
    )
    ...
    for path_key in [
        ...
        assert config[path_key] is not None, (
            f"{path_key} must be specified in config file."
        )

        if isinstance(config[path_key], list):
            env_vars = [
                re.findall(r"(?:^|\/)\$(\w+)(?:\/|$)", d) for d in config[path_key]
            ]
            for file_set_idx in range(len(env_vars)):
                for env_var in env_vars[file_set_idx]:
                    if env_var in os.environ:
                        config[path_key][file_set_idx] = config[path_key][
                            file_set_idx
                        ].replace(f"${env_var}", os.environ[env_var])
        else:
            env_vars = re.findall(r"(?:^|\/)\$(\w+)(?:\/|$)", config[path_key])
            for env_var in env_vars:
                if env_var in os.environ:
                    config[path_key] = config[path_key].replace(
    ...
    for fp in config["raw_data_file_signature"]:
        if fp.endswith(".nc"):
            ...
            raise ValueError(
                "Invalid file signature. Please specify either '*.nc', '*.csv', or '*.parquet'."
            )
    ...
    if args.reload_data and args.verbose and "netcdf" in config["data_format"]:
        data_loader.print_netcdf_structure(
            data_loader.file_paths[config["data_format"].index("netcdf")][0]
        )
    ...
    if not args.reload_data and os.path.exists(data_loader.save_path):
        ...
        df_query = df_query.select(
            [pl.col("time"), pl.col("file_set_idx")]
            + [
                pl.col(c)
                for c in sorted(
                    df_query.select(cs.numeric())
                    .select(pl.exclude("file_set_idx"))
                    .collect_schema()
                    .names(),
                    key=lambda col: (
                        re.search(f".*?(?={data_loader.turbine_signature})", col).group(
                            0
                        ),
                        int(
                            re.search(
                                "\\d+",
                                re.search(data_loader.turbine_signature, col).group(0),
                            ).group(0)
                        ),
                    ),
                )
        ...
        data_loader.turbine_ids = data_loader.get_turbine_ids(
            data_loader.turbine_signature, df_query, sort=True
        )
        if data_loader.turbine_mapping is None:
            data_loader.turbine_mapping = [
                {
                    k: k
                    for k in [
                        re.findall("(?<=wind_speed_)(.+)", n)[0]
                        for n in df_query.select(cs.starts_with("wind_speed_"))
                        .collect_schema()
                        .names()
                    ]
                }
        ...
        if args.multiprocessor == "cf":
            max_workers = int(
                os.environ.get("MAX_WORKERS", multiprocessing.cpu_count())
            )
        ...
        logging.info(
            "🔄 Processing new data files with %d files",
            sum(len(fp) for fp in data_loader.file_paths),
        )
        start_time = time.time()

    if args.reload_data or not os.path.exists(data_loader.save_path):
        temp_save_dir = os.path.join(
            os.path.dirname(data_loader.save_path),
            os.path.basename(data_loader.save_path).replace(".parquet", "_temp"),
        )
        ...
        logging.info(
            f"Making directory to save_path {os.path.dirname(data_loader.save_path)}"
        )
        os.makedirs(os.path.dirname(data_loader.save_path), exist_ok=True)

        df_query = data_loader.read_multi_files(
            temp_save_dir, read_single_files=False
        )  # TODO this should be set to all or unprocessed or False, and should change if there are no processed files
        ...
        logging.info(
            "✅ Finished reading individual files. Time elapsed: %.2f s",
    ...
    assert all(
        any(prefix in col for col in df_query.collect_schema().names())
        for prefix in [
            "time",
            "wind_speed_",
            "wind_direction_",
        ...
        "DataFrame must contain columns 'time', then columns with prefixes 'wind_speed_', 'wind_direction_', 'power_output_', 'nacelle_direction_'"
    )
    assert df_query.select("time").collect().to_series().is_sorted(), (
        "Loaded data should be sorted by time!"
    )
    assert all(
        any(
            f"{prefix}{tid}" in col
            for col in df_query.collect_schema().names()
            if col not in ["time", "file_set_idx"]
        )
        for prefix in [
            "wind_speed_",
            "wind_direction_",
        ...
        "DataFrame must contain columns with prefixes 'wind_speed_', 'wind_direction_', 'power_output_', 'nacelle_direction_' and suffixes for each turbine id"
    )
    ...
    if args.debug:
        df_query = (
            df_query.select(
                ["time", "file_set_idx"]
                + [
                    cs.ends_with(f"wt{i + 1:03d}")
                    for i in set(
                        np.concatenate(config["nacelle_calibration_turbine_pairs"])
                    )
                ]
            )
            .slice(
                ...
                int(
                    ...
                    * 30
                    * np.timedelta64(1, "D")
                    / np.timedelta64(data_loader.dt, "s")
                ),
            )
            .collect()
            .lazy()
        )
        df_query.sink_parquet(
            "/Users/ahenry/Documents/toolboxes/wind-forecasting/examples/data/awaken_data/awaken_data_processed_debug.parquet"
        )
        df_query = pl.scan_parquet(
            "/Users/ahenry/Documents/toolboxes/wind-forecasting/examples/data/awaken_data/awaken_data_processed_debug.parquet"
        )
    ...
    if args.plot:
        ...
        waked_distances = np.array(
            [
                (
                    (
                        data_inspector.fmodel.layout_x[pair[0]]
                        - data_inspector.fmodel.layout_x[pair[1]]
                    )
                    ** 2
                    + (
                        data_inspector.fmodel.layout_y[pair[0]]
                        - data_inspector.fmodel.layout_y[pair[1]]
                    )
                    ** 2
                )
                ** 0.5
                for pair in config["nacelle_calibration_turbine_pairs"] + [[74, 73]]
            ]
        )
        ...
        if "file_set_idx" in df_query.collect_schema().names():
            file_set_indices = (
                df_query.select("file_set_idx").unique().collect().to_numpy().flatten()
            )
            ...
            if False:
                ROW_LIMIT = None
                data_inspector.plot_wind_rose(
                    df_query2.slice(0, ROW_LIMIT),
                    feature_type="wind_direction",
                    turbine_ids=["wt075", "wt074"],
                    fig_label=f"wind_rose_all",
                )

                data_inspector.plot_wind_rose(
                    df_query2.slice(0, ROW_LIMIT),
                    feature_type="nacelle_direction",
                    turbine_ids=["wt075", "wt074"],
                    fig_label=f"nacelle_rose_all",
                )

                data_inspector.plot_time_series(
                    df_query2.slice(0, ROW_LIMIT),
                    feature_types=["wind_speed", "wind_direction", "power_output"],
                    turbine_ids=["wt075", "wt074"],
            ...
            if False:
                for file_set_idx in file_set_indices:
                    data_inspector.plot_wind_rose(
                        df_query2.filter(pl.col("file_set_idx") == file_set_idx).slice(
                            0, ROW_LIMIT
                        ),
                        feature_type="wind_direction",
                        turbine_ids="all",
                        fig_label=f"wind_rose_{file_set_idx}",
                    )
                    ...
                    data_inspector.plot_wind_rose(
                        ...
                        feature_type="nacelle_direction",
                        turbine_ids="all",
                        fig_label=f"nacelle_rose_{file_set_idx}",
                    )

                # NOTE: USE THIS CODE TO GENERATE FIG. 5 IN PAPER, perhaps without row_limit
                for file_set_idx in file_set_indices:
                    data_inspector.plot_wind_speed_weibull(
                        df_query2.filter(pl.col("file_set_idx") == file_set_idx).slice(
                            0, ROW_LIMIT
                        ),
                        turbine_ids="all",
                        fig_label=f"weibull_{file_set_idx}",
                    )

            if False:
                for file_set_idx in file_set_indices:
                    data_inspector.plot_wind_speed_power(
                        df_query2.filter(pl.col("file_set_idx") == file_set_idx).slice(
                            0, ROW_LIMIT
                        ),
                        turbine_ids=data_loader.turbine_ids,
                        fig_label=f"power_curve_{file_set_idx}",
                    )

                for file_set_idx in file_set_indices:
                    fig, _ = plot.column_histograms(
                        data_inspector.collect_data(
                            df=df_query2.filter(
                                pl.col("file_set_idx") == file_set_idx
                            ).slice(0, ROW_LIMIT),
                    ...
                    fig.savefig(
                        os.path.join(
                            data_inspector.save_dir,
                            f"wind_speed_histogram_{file_set_idx}.png",
                        )
                    )
                    fig, _ = plot.column_histograms(
                        data_inspector.collect_data(
                    ...
                    fig.savefig(
                        os.path.join(
                            data_inspector.save_dir,
                            f"wind_dir_histogram_{file_set_idx}.png",
                        )
                    )

        if False:
            df_query.with_columns(
                pl.col("time").dt.round(f"{10}m").alias("time")
            ).group_by("time", maintain_order=True).agg(cs.numeric().mean()).select(
                "time",
                "file_set_idx",
                cs.starts_with("wind_speed"),
                cs.starts_with("wind_direction"),
            ).unpivot(
                on=cs.starts_with("wind_speed"),
                index=["time", "file_set_idx", cs.starts_with("wind_direction")],
            ).filter(pl.col("value").is_between(0, 25, closed="both")).collect(
                engine="streaming"
            ).pivot(
                on="variable",
                values="value",
                ...
                os.path.join(
                    ...
                    os.path.basename(config["processed_data_path"]).replace(
                        ".parquet", "_10min.parquet"
                    ),
                )
            )
        ...
        data_inspector.plot_wind_rose(
            df_query2.slice(0, None),
            feature_type="wind_direction",
            turbine_ids="all",
            fig_label=f"wind_rose",
        )
        data_inspector.plot_wind_speed_weibull(
            df_query2.slice(0, None), turbine_ids="all", fig_label=f""
        )
    ...
    # %% check time series
    if args.verbose:
        DataInspector.print_df_state(
            df_query, ["wind_speed", "wind_direction", "nacelle_direction"]
        )
    if args.plot:
        data_inspector.plot_time_series(
            df_query.slice(0, ROW_LIMIT),
    ...
    file_set_indices = (
        df_query.select(pl.col("file_set_idx").unique()).collect().to_numpy().flatten()
    )
    ...
    if args.apply_filters:
        if "unresponsive_sensor" in config["filters"]:
            ...
            frozen_sensor_filter_target_path = os.path.join(
                os.path.dirname(config["processed_data_path"]),
                os.path.basename(config["processed_data_path"]).replace(
                    ".parquet", "_frozen_sensor"
                ),
            )
            ...
            regen = (
                args.reload_data
                or args.regenerate_filters
                or not all(
                    os.path.exists(
                        os.path.join(
                            frozen_sensor_filter_target_path,
                            f"{feat}_fs{file_set_idx}.npy",
                        )
                    )
                    for file_set_idx in file_set_indices
                    for feat in cols
            ...
            if regen or not all(
                os.path.exists(
                    os.path.join(
                        frozen_sensor_filter_target_path, f"{feat}_fs{file_set_idx}.npy"
                    )
                )
                for file_set_idx in file_set_indices
                for feat in cols
                ...
                thr = int(
                    np.timedelta64(
                        config["filters"]["unresponsive_sensor"]["frozen_sensor_limit"],
                        "s",
                    )
                    / np.timedelta64(data_loader.dt, "s")
                )
                ...
                for file_set_idx in file_set_indices:
                    if regen or not all(
                        ...
                        logging.info(
                            f"Regenerating frozen sensor mask for file_set {file_set_idx}."
                        )
                        flag = filters.unresponsive_flag(
                            data_pl=df_query.filter(
                                pl.col("file_set_idx") == file_set_idx
                            ).select(
                                cs.starts_with("wind_speed"),
                                cs.starts_with("wind_direction"),
                        ...
                        for feat in cols:
                            logging.info(
                                f"Saving frozen sensor mask for file_set {file_set_idx}, feature {feat}."
                            )
                        ...
                        logging.info(
                            f"Loading existing frozen sensor mask for file_set {file_set_idx}."
                        )

            mask = lambda file_set_idx, feat: pl.Series(
                np.load(
                    os.path.join(
                        frozen_sensor_filter_target_path, f"{feat}_fs{file_set_idx}.npy"
                    )
            ...
            # check time series
            if args.verbose:
                DataInspector.print_pc_remaining_vals(
                    df_query,
                    mask,
                    file_set_indices,
            ...
            if args.plot:
                ...
                data_inspector.plot_nulled_vs_remaining(
                    df_query.slice(0, ROW_LIMIT),
                    mask,
                    file_set_indices,
                    ...
                    feature_types=["wind_speed"],
                    feature_labels=[
                        "Wind Speed (m/s) after Unresponsive Sensor Filter"
                    ],
                )
                data_inspector.plot_nulled_vs_remaining(
                    df_query.slice(0, ROW_LIMIT),
                    ...
                    feature_types=["wind_direction"],
                    feature_labels=[
                        "Wind Direction ($^\\circ$) after Unresponsive Sensor Filter"
                    ],
                )
            ...
            threshold = 0.01
            logging.info(
                "Nullifying wind speed/direction frozen sensor measurements in dataframe."
            )

            df_query = data_filter.conditional_filter(
                df_query,
                threshold,
                mask,
                file_set_indices,
            ...
            df_query.collect(engine="streaming").write_parquet(
                config["processed_data_path"].replace(".parquet", "_filtered.parquet")
            )
            df_query = pl.scan_parquet(
                config["processed_data_path"].replace(".parquet", "_filtered.parquet")
            )
            logging.info(
                "Finished nullifying wind speed/direction frozen sensor measurements in dataframe."
            )
            ...
            if args.plot:
                data_inspector.plot_time_series(
                    ...
                    turbine_ids=data_loader.turbine_ids,
                    continuity_groups=None,
                    label="after_frozen_sensor",
                )

        # %%
        if "inoperational" in config["filters"] and any(
            col.startswith("turbine_status")
            for col in df_query.collect_schema()["names"]
        ):  # 10 is normal operation for AWAKEN
            ...
            status_codes = [1]
            mask = lambda tid: (
                ~pl.col(f"turbine_status_{tid}").is_in(status_codes)
                & pl.col(f"turbine_status_{tid}").is_not_null()
            )
            ...
            logging.info(
                "Finished nullifying inoperational turbine measurements in dataframe."
            )
        ...
        if "range_flag" in config["filters"]:
            ...
            if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
                ...
                out_of_range = (
                    filters.range_flag(
                        ...
                        upper=config["filters"]["range_flag"]["upper"],
                    )
                    & ~ws.isna()
                ).values  # range flag includes formerly null values as nan
                del ws
            ...
            mask = lambda tid: pl.Series(out_of_range[:, turbine_id_to_index[tid]])
            ...
            if args.plot:
                ...
                fig, axs = plot.plot_power_curve(
                    ...
                    .to_numpy()[:, target_turbine_idx]
                    .flatten(),
                    data_inspector.collect_data(
                        df=df_query, feature_types="power_output"
                    )
                    .to_numpy()[:, target_turbine_idx]
                    .flatten(),
                ...
                fig.savefig(
                    os.path.join(
                        data_inspector.save_dir, "wind_speed_out_of_range_awaken.png"
                    ),
                    dpi=100,
                )
            ...
            logging.info(
                "Nullifying wind speed out of range measurements in dataframe."
            )
            ...
            logging.info(
                "Finished nullifying wind speed out of range measurements in dataframe."
            )
        ...
        if "window_range_flag" in config["filters"]:
            ...
            if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
                ...
                out_of_window = data_filter.multi_generate_filter(
                    df_query=df_query,
                    filter_func=data_filter._single_generate_window_range_filter,
                    ...
                    value_min=config["filters"]["window_range_flag"]["value_min"]
                    * data_inspector.rated_turbine_power,
                    value_max=config["filters"]["window_range_flag"]["value_max"]
                    * data_inspector.rated_turbine_power,
                )
            ...
            if args.plot:
                data_inspector.plot_nulled_vs_remaining(
                    ...
                    feature_types=["wind_speed"],
                    feature_labels=[
                        "Wind Speed (m/s) after Wind Speed-Power Out-of-Window Filter"
                    ],
                )
                ...
                fig.savefig(
                    os.path.join(
                        data_inspector.save_dir, "power_curve_out_of_window_range.png"
                    ),
                    dpi=100,
                )
            ...
            logging.info(
                "Nullifying wind speed-power curve out-of-window measurements in dataframe."
            )
            ...
            logging.info(
                "Finished nullifying wind speed-power curve out-of-window measurements in dataframe."
            )
        ...
        if "bin_filter" in config["filters"]:
            ...
            if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
                ...
                bin_outliers = data_filter.multi_generate_filter(
                    df_query=df_query,
                    filter_func=data_filter._single_generate_bin_filter,
                    feature_types=["wind_speed", "power_output"],
                    turbine_ids=data_loader.turbine_ids,
                    ...
                    bin_min=config["filters"]["bin_filter"]["bin_min"]
                    * data_inspector.rated_turbine_power,
                    bin_max=config["filters"]["bin_filter"]["bin_max"]
                    * data_inspector.rated_turbine_power,
            ...
            # check if wind speed/dir measurements from inoperational turbines differ from fully operational
            mask = lambda tid: pl.Series(bin_outliers[:, turbine_id_to_index[tid]])
            ...
            if args.plot:
                data_inspector.plot_nulled_vs_remaining(
                    ...
                    feature_types=["wind_speed"],
                    feature_labels=[
                        "Wind Speed (m/s) after Wind Speed-Power Bin Outlier Filter"
                    ],
                )
                ...
                if False:
                    ...
                    bin_outliers = np.load(
                        config["processed_data_path"].replace(
                            ".parquet", "_bin_outliers.npy"
                        )
                    )
                    out_of_window = np.load(
                        config["processed_data_path"].replace(
                            ".parquet", "_out_of_window.npy"
                        )
                    )
                    ...
                    target_turbine_id = list(data_loader.turbine_mapping[0].values())[
                        target_turbine_idx
                    ]

                    df_query2 = (
                        df_query.with_row_index()
                        .select(
                            [
                                f"wind_speed_{target_turbine_id}",
                                f"power_output_{target_turbine_id}",
                            ]
                        )
                        .filter(
                    ...
                    slc = slice(
                        df_query2.select(pl.col("index").first()).item(),
                        df_query2.select(pl.col("index").last()).item(),
                    )
                    # .filter(pl.col("time").is_between(lower_bound=ROW_BOUNDS[0], upper_bound=ROW_BOUNDS[1], closed="both"))
                    ...
                    fig, axs = plot.plot_power_curve(
                        ...
                        flag=(
                            bin_outliers[:, target_turbine_idx]
                            | out_of_window[:, target_turbine_idx]
                        )[slc],
                    ...
                    fig.savefig(
                        os.path.join(
                            data_inspector.save_dir,
                            "power_curve_bin_outliers_awaken.png",
                        ),
                        dpi=100,
                    )
            ...
            threshold = 0.01
            logging.info(
                "Nullifying wind speed-power curve bin outlier measurements in dataframe."
            )
            ...
            logging.info(
                "Finished nullifying wind speed-power curve bin outlier measurements in dataframe."
            )
    ...
    if "nacelle_calibration" in config["filters"] and (
        args.reload_data
        or args.regenerate_filters
        or not os.path.exists(
            config["processed_data_path"].replace(".parquet", "_calibrated_2.parquet")
        )
    ):
        # if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
        ...
        logging.info(
            "Subtracting median wind direction from wind direction and nacelle direction measurements."
        )
        offset = 2.0
        ...
        if use_existing_corrections:
            final_corrections = pl.read_csv(
                os.path.join(
                    os.path.dirname(config["processed_data_path"]), "corrections.csv"
                )
            )

            for row in final_corrections.iter_rows(named=True):
                turbine_id = f"wt{row['Turbine ID']:03d}"
                corrections = list(
                    set([v for k, v in row.items() if "Northing Bias" in k])
                )
                ...
                if len(corrections) == 1:
                    expr = [
                        ...
                        .mod(360.0)
                        .alias(f"wind_direction_{turbine_id}"),
                        (
                            pl.col(f"nacelle_direction_{turbine_id}")
                            + corrections[0]
                            - offset
                        )
                        .mod(360.0)
                        .alias(f"nacelle_direction_{turbine_id}"),
                    ]
                else:
                    switch_times = re.findall(
                        r"\d{1,2}/\d{1,2}/\d{4}\s+\d{1,2}:\d{2}\s+UTC", row["Notes"]
                    )
                    expr = None

                    for s, st in enumerate(switch_times):
                        st = datetime.strptime(
                            " ".join(st.split()), "%m/%d/%Y %H:%M %Z"
                        )
                        if expr is None:
                            expr = [
                                pl.when(pl.col("time") < st).then(
                                    pl.col(f"wind_direction_{turbine_id}")
                                    + corrections[s]
                                ),
                                pl.when(pl.col("time") < st).then(
                    ...
                    expr[0] = (
                        expr[0]
                        .otherwise(
                            pl.col(f"wind_direction_{turbine_id}") + corrections[-1]
                        )
                        .mod(360.0)
                        .alias(f"wind_direction_{turbine_id}")
                    )
                    expr[1] = (
                        expr[1]
                        .otherwise(
                            pl.col(f"nacelle_direction_{turbine_id}") + corrections[-1]
                        )
                        .mod(360.0)
                        .alias(f"nacelle_direction_{turbine_id}")
                    )
            ...
            if (
                ...
                wd_median = df_query_10min.select(
                    cs.starts_with("wind_direction")
                    .radians()
                    .sin()
                    .name.suffix("_sin"),
                ...
                wd_median = pl.concat(
                    [
                        wd_median.select(
                            pl.concat_list(cs.ends_with("_sin"))
                            .list.drop_nulls()
                            .list.median()
                            .alias("wd_sin_median")
                        ),
                        wd_median.select(
                            pl.concat_list(cs.ends_with("_cos"))
                            .list.drop_nulls()
                            .list.median()
                            .alias("wd_cos_median")
                        ),
                    ...
                    pl.arctan2(pl.col("wd_sin_median"), pl.col("wd_cos_median"))
                    .degrees()
                    .alias("wd_median")
                )

                # fetching median sin/cos abs nacelle dir and computing arctan of that
                nd_median = df_query_10min.select(
                    cs.starts_with("nacelle_direction")
                    .radians()
                    .sin()
                    .name.suffix("_sin"),
                    cs.starts_with("nacelle_direction")
                ...
                nd_median = pl.concat(
                    ...
                    pl.arctan2(pl.col("nd_sin_median"), pl.col("nd_cos_median"))
                    .degrees()
                    .alias("nd_median")
                )

                # df_query_10min = df_query_10min.with_columns(wd_median=wd_median, yaw_median=yaw_median).collect().lazy()
                df_query_10min = pl.concat(
                    [df_query_10min, wd_median, nd_median], how="horizontal"
                )

                del wd_median, nd_median

                if args.plot:
                    data_inspector.plot_wind_offset(
                        df_query_10min, "Original", data_loader.turbine_ids
                    )

                # remove biases from median direction
                logging.info("Started sinking dataframe.")
                df_query_10min.collect(engine="streaming").write_parquet(
                    config["processed_data_path"].replace(
                        ".parquet", "_calibrated_1.parquet"
                    )
                )
            ...
            df_query_10min = pl.scan_parquet(
                config["processed_data_path"].replace(
                    ".parquet", "_calibrated_1.parquet"
                )
            )
            ...
            if (
                ...
                or args.regenerate_filters
                or not os.path.exists(
                    config["processed_data_path"].replace(".parquet", "_biases.npy")
                )
            ):
                # data_filter.multiprocessor = None
                biases = data_filter.multi_compute_bias(
                    df_query_10min, data_loader.turbine_ids
                )
                # data_filter.multiprocessor = args.multiprocessor
            ...
            if args.verbose and False:
                ...
                for turbine_id in data_loader.turbine_ids:
                    turbine_bias = (
                        df_query_10min.filter(pl.col(f"power_output_{turbine_id}") >= 0)
                        .select(
                            "time",
                            f"wind_direction_{turbine_id}",
                            f"nacelle_direction_{turbine_id}",
                        )
                        .select(
                            bias=(
                                pl.col(f"wind_direction_{turbine_id}")
                        ...
                        .select(
                            sin=pl.all().radians().sin().mean(),
                            cos=pl.all().radians().cos().mean(),
                        )
                        .select(
                            pl.arctan2(pl.col("sin"), pl.col("cos"))
                            .degrees()
                            .mod(360.0)
                            .alias("bias")
                        )
                        .select(
                            pl.when(pl.all() > 180.0)
                            .then(pl.all() - 360.0)
                            .otherwise(pl.all())
                        )
                        .collect()
                ...
                logging.info(
                    f"Average Bias = {bias / len(data_loader.turbine_ids)} deg"
                )
            ...
            dir_offsets = compute_offsets(
                df_query_10min,
                data_inspector.fmodel,
                turbine_ids=data_loader.turbine_ids,
                turbine_pairs=config["nacelle_calibration_turbine_pairs"],
                plot=[(18, 19)],  # args.plot,
                save_path=os.path.join(
                    os.path.dirname(config["processed_data_path"]), "pre_correction.png"
                ),
            )
            ...
            for bias, turbine_id in zip(biases, data_loader.turbine_ids):
                logging.info(
                    f"Turbine {turbine_id} bias from median wind direction: {bias} deg, from power_ratio: {dir_offsets} deg, total = {bias + dir_offsets}"
                )
                ...
                if turbine_id in ["wt033", "wt042", "wt078"]:
                    if turbine_id == "wt033":
                        switch_time = datetime.strptime(
                            "11/5/2024 14:06", "%m/%d/%Y %H:%M"
                        )
                        bias_1, bias_2 = 8.3, 106.8

                    elif turbine_id == "wt042":
                        switch_time = datetime.strptime(
                            "4/12/2025 21:07", "%m/%d/%Y %H:%M"
                        )
                        bias_1, bias_2 = 7, -54.4
                    elif turbine_id == "wt078":
                        switch_time = datetime.strptime(
                            "8/19/2024 13:07", "%m/%d/%Y %H:%M"
                        )
                        bias_1, bias_2 = -6.7, 36.7

                    expr = (
                        pl.when(pl.col("time") < switch_time)
                        .then(pl.col(f"wind_direction_{turbine_id}") - bias_1)
                        .otherwise(pl.col(f"wind_direction_{turbine_id}") - bias_2)
                        .mod(360.0)
                        .alias(f"wind_direction_{turbine_id}"),
                        pl.when(pl.col("time") < switch_time)
                        .then(pl.col(f"nacelle_direction_{turbine_id}") - bias_1)
                        .otherwise(pl.col(f"nacelle_direction_{turbine_id}") - bias_2)
                        .mod(360.0)
                        .alias(f"nacelle_direction_{turbine_id}"),
                    )
                else:
                    bias += dir_offsets
                    expr = (
                        (pl.col(f"wind_direction_{turbine_id}") - bias)
                        .mod(360.0)
                        .alias(f"wind_direction_{turbine_id}"),
                        (pl.col(f"nacelle_direction_{turbine_id}") - bias)
                        .mod(360.0)
                        .alias(f"nacelle_direction_{turbine_id}"),
                    )
            ...
            if args.plot:
                data_inspector.plot_wind_offset(
                    df_query_10min, "Corrected", data_loader.turbine_ids
                )
        ...
        if args.plot:
            data_inspector.plot_time_series(
                ...
                turbine_ids=data_loader.turbine_ids,
                continuity_groups=None,
                label="after_nacelle_calibration",
            )
    else:
        df_query = pl.scan_parquet(
            config["processed_data_path"].replace(".parquet", "_calibrated_2.parquet")
    ...
    if (
        ...
        or not (
            os.path.exists(
                config["processed_data_path"].replace(".parquet", "_stddev.parquet")
            )
            or os.path.exists(
                config["processed_data_path"].replace(".parquet", "_split.parquet")
            )
            or os.path.exists(
                config["processed_data_path"].replace(".parquet", "_imputed.parquet")
            )
        )
    ):
        ...
        df_query2 = (
            df_query.with_columns(
                **{
                    f"ws_horz_{tid}": (
                        pl.col(f"wind_speed_{tid}")
                        * ((pl.col(f"wind_direction_{tid}") + 180.0).radians().sin())
                    )
                    for tid in data_loader.turbine_ids
                }
            )
            .with_columns(
                **{
                    f"ws_vert_{tid}": (
                        pl.col(f"wind_speed_{tid}")
                        * ((pl.col(f"wind_direction_{tid}") + 180.0).radians().cos())
                    )
                    for tid in data_loader.turbine_ids
                }
            )
            .with_columns(
                **{
                    f"nd_cos_{tid}": (
                        (pl.col(f"nacelle_direction_{tid}")).radians().cos()
                    )
                    for tid in data_loader.turbine_ids
                }
            )
            .with_columns(
                **{
                    f"nd_sin_{tid}": (
                        (pl.col(f"nacelle_direction_{tid}")).radians().sin()
                    )
                    for tid in data_loader.turbine_ids
                }
            )
            .select(
                pl.col("time"),
                pl.col("file_set_idx"),
                cs.starts_with("ws_horz"),
                cs.starts_with("ws_vert"),
                cs.starts_with("nd_sin"),
                cs.starts_with("nd_cos"),
                cs.starts_with("power_output"),
            )
        ...
        if False:
            wind_dirs = np.arctan2(
                df_query2.select(cs.starts_with("ws_horz")).collect().to_numpy(),
                df_query2.select(cs.starts_with("ws_vert")).collect().to_numpy(),
            ) * (180.0 / np.pi)
            wind_dirs[wind_dirs < 0] = 360.0 + wind_dirs[wind_dirs < 0]
            wind_dirs = np.mod(wind_dirs, 360.0)
            assert np.allclose(
                df_query.select(cs.starts_with("wind_direction")).collect().to_numpy(),
                wind_dirs,
            )
            wind_mags = (
                df_query2.select(cs.starts_with("ws_horz")).collect().to_numpy() ** 2
                + df_query2.select(cs.starts_with("ws_vert")).collect().to_numpy() ** 2
            ) ** 0.5
            assert np.allclose(
                df_query.select(cs.starts_with("wind_speed")).collect().to_numpy(),
                wind_mags,
            )
    ...
    if args.plot:
        ...
        if "continuity_group" in df_query.collect_schema().names():
            continuity_groups = (
                df_query.select(pl.col("continuity_group"))
                .unique()
                .collect()
                .to_numpy()
                .flatten()
            ...
            data_inspector.plot_time_series(
                pl.concat(
                    [
                        df.slice(0, ROW_LIMIT)
                        for df in df_query.collect().partition_by("continuity_group")
                    ],
                    how="vertical",
                ).lazy(),
    ...
    if "std_range_flag" in config["filters"]:
        ...
        if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
            ...
            std_dev_filter_target_path = os.path.join(
                os.path.dirname(config["processed_data_path"]),
                os.path.basename(config["processed_data_path"]).replace(
                    ".parquet", "_std_dev_outliers"
                ),
            )
            ...
            cols = (
                df_query.select(cs.starts_with("ws_horz"), cs.starts_with("ws_vert"))
                .collect_schema()
                .names()
            )
            if config["filters"]["std_range_flag"]["over"] == "asset":
                ...
                chunk_size = (
                    1_000_000 * len(cols)
                )  #  total_rows * 2 # process a number of cells equal to the twice total row number at a time ,1_000_000_000
            ...
            if (
                ...
                or (
                    not all(
                        os.path.exists(
                            os.path.join(std_dev_filter_target_path, f"{s}.parquet")
                        )
                        for s in filenames
                    )
                )
                ...
                if config["filters"]["std_range_flag"]["over"] == "asset":
                    ...
                    for feat_type in feature_types:
                        if not args.regenerate_filters and os.path.exists(
                            os.path.join(
                                std_dev_filter_target_path, f"corr_{feat_type}.parquet"
                            )
                        ):
                            logging.info(
                                f"Found existing file for std_dev_outliers correlation matrix. Used {virtual_memory().percent}% of RAM."
                            )
                            corr_df[feat_type] = pl.read_parquet(
                                os.path.join(
                                    std_dev_filter_target_path,
                                    f"corr_{feat_type}.parquet",
                                )
                            )
                        else:
                            logging.info(
                                f"Started generating correlation matrix for feature type {feat_type}. Used {virtual_memory().percent}% of RAM."
                            )
                            corr_df[feat_type] = asset_correlation_matrix_pl(
                                df_query.select(
                                    cs.starts_with("ws_horz"), cs.starts_with("ws_vert")
                                ),
                                feat_type,
                            )
                            corr_df[feat_type].write_parquet(
                                os.path.join(
                                    std_dev_filter_target_path,
                                    f"corr_{feat_type}.parquet",
                                )
                            ...
                            logging.info(
                                f"Finished generating correlation matrix for feature type {feat_type}. Used {virtual_memory().percent}% of RAM."
                            )
                    ...
                    for s, start_row in enumerate(range(0, total_rows, row_chunk_size)):
                        ...
                        if not args.regenerate_filters and os.path.exists(
                            os.path.join(
                                std_dev_filter_target_path, f"chunk_{s}.parquet"
                            )
                        ):
                            logging.info(
                                f"Found existing file for rows {start_row} to {end_row} of {total_rows} of std_dev_outliers. Used {virtual_memory().percent}% of RAM."
                            )
                            continue

                        logging.info("")
                        logging.info(
                            f"Started generating flag for rows {start_row} to {end_row} of {total_rows} of std_dev_outliers with number of threads {pl.thread_pool_size(), os.environ.get('POLARS_MAX_THREADS', None), Process().num_threads()}."
                        )

                        df = (
                            df_query.slice(start_row, end_row - start_row)
                            .select(
                                cs.starts_with("ws_horz"), cs.starts_with("ws_vert")
                            )
                            .collect()
                        ...
                        df = filters.std_range_flag(
                            ...
                            over=config["filters"]["std_range_flag"][
                                "over"
                            ],  # asset or time
                            feature_types=["ws_horz", "ws_vert"],
                            r2_threshold=config["filters"]["std_range_flag"][
                                "r2_threshold"
                            ],
                            min_correlated_assets=config["filters"]["std_range_flag"][
                        ...
                        logging.info(
                            f"Finished generating/writing flag for rows {start_row} to {end_row} of {total_rows} of std_dev_outliers."
                        )
                    del df
                else:
                    for c, col in enumerate(cols):
                        if not args.regenerate_filters and os.path.exists(
                            os.path.join(std_dev_filter_target_path, f"{c}.parquet")
                        ):
                            used_ram = virtual_memory().percent
                            logging.info(
                                f"Found existing file for column {c} of {len(cols)} of std_dev_outliers. Used {used_ram}% of RAM."
                            )
                            continue
                        ...
                        filters.std_range_flag(
                            ...
                            feature_types=[
                                re.search(
                                    f"\\w+(?=_{data_loader.turbine_signature})", col
                                ).group()
                            ],
                            r2_threshold=config["filters"]["std_range_flag"][
                                "r2_threshold"
                            ...
                            os.path.join(
                                std_dev_filter_target_path, f"chunk_{c}.parquet"
                            )
                        )

                        logging.info(
                            f"Processing column {c} of {len(cols)} of std_dev_outliers. Maximum RAM used was {max_ram}%."
                        )

            if config["filters"]["std_range_flag"]["over"] == "asset":
                std_dev_outliers = pl.scan_parquet(
                    os.path.join(std_dev_filter_target_path, "chunk_*.parquet")
                )
                ...
                mask = lambda feat: pl.read_parquet(
                    os.path.join(std_dev_filter_target_path, f"{feat}.parquet")
                ).to_series()
            ...
            if args.plot:
                data_inspector.plot_nulled_vs_remaining(
                    ...
                    feature_labels=[
                        "Horizontal Wind Speed (m/s) after Standard Deviation Filter"
                    ],
                )
                ...
                data_inspector.plot_nulled_vs_remaining(
                    ...
                    feature_labels=[
                        "Vertical Wind Speed (m/s) after Standard Deviation Filter"
                    ],
                )
            ...
            logging.info(
                "Nullifying horizontal/vertical wind speed standard deviation measurements in dataframe."
            )
            ...
            df_query.collect(engine="streaming").write_parquet(fp)
            logging.info(
                "Finished nullifying horizontal/vertical wind speed standard deviation measurements in dataframe."
            )
        else:
        ...
        if args.verbose:
            DataInspector.print_df_state(
                df_query, ["ws_horz", "ws_vert", "nd_cos", "nd_sin"]
            )
    ...
    if "split" in config["filters"]:
        ...
        if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
            logging.info(
                "Split dataset during time steps for which many turbines have missing data."
            )
            ...
            assert config["filters"]["split"]["missing_col_thr"] <= len(
                data_loader.turbine_ids
            )
            missing_col_thr = config["filters"]["split"]["missing_col_thr"]
            missing_duration_thr = np.timedelta64(
                config["filters"]["split"]["missing_duration_thr"], "s"
            )
            minimum_not_missing_duration = np.timedelta64(
                config["filters"]["split"]["minimum_not_missing_duration"], "s"
            )
            missing_data_cols = ["ws_horz", "ws_vert"]

            # file_set_indices = [0]
            df_query = [
                df_query.filter(pl.col("file_set_idx") == file_set_idx)
                for file_set_idx in file_set_indices
            ]
            max_cg = None
            ...
            for f, file_set_idx in enumerate(file_set_indices):
                ...
                file_set_fp = os.path.join(
                    dirpath,
                    os.path.basename(config["processed_data_path"]).replace(
                        ".parquet", f"_split_fs{file_set_idx}.parquet"
                    ),
                )
                if not args.regenerate_filters and os.path.exists(file_set_fp):
                    max_cg = (
                        pl.scan_parquet(file_set_fp)
                        .select(pl.col("continuity_group").max())
                        .collect()
                        .item()
                    )
                    logging.info(
                        f"Found existing split dataframe for file set {file_set_idx} with maximum continuity_group {max_cg}."
                    )
                else:
                    not_missing_fp = os.path.join(
                        dirpath,
                        os.path.basename(config["processed_data_path"]).replace(
                            ".parquet", f"_not_missing_times_fs{file_set_idx}.parquet"
                        ),
                    )
                    missing_fp = os.path.join(
                        dirpath,
                        os.path.basename(config["processed_data_path"]).replace(
                            ".parquet", f"_missing_times_fs{file_set_idx}.parquet"
                        ),
                    )
                    ...
                    df_query[f] = (
                        ...
                        .with_columns(
                            *[
                                cs.contains(col).is_null().name.prefix("is_missing_")
                                for col in missing_data_cols
                            ]
                        )
                        .with_columns(
                            **{
                                f"num_missing_{col}": pl.sum_horizontal(
                                    (cs.contains(col) & cs.starts_with("is_missing"))
                                )
                                for col in missing_data_cols
                            }
                        )
                    ...
                    logging.info("")
                    logging.info(
                        f"Generating missing/not missing flags for file set {file_set_idx}."
                    )
                    ...
                    df_query_not_missing_times = add_df_continuity_columns(
                        ...
                        mask=pl.all_horizontal(
                            cs.starts_with("num_missing") <= missing_col_thr
                        ),
                    ...
                    df_query_not_missing = add_df_agg_continuity_columns(
                        df_query_not_missing_times
                    )
                    df_query_missing = add_df_agg_continuity_columns(
                        df_query_missing_times
                    )

                    os.remove(not_missing_fp) if os.path.exists(
                        not_missing_fp
                    ) else None
                    os.remove(missing_fp) if os.path.exists(missing_fp) else None
                    df_query_not_missing.collect().write_parquet(not_missing_fp)
                    df_query_missing.collect().write_parquet(missing_fp)
                    ...
                    df_query_not_missing = pl.concat(
                        [
                            df_query_not_missing,
                            df_query_missing.filter(
                                pl.col("duration") <= missing_duration_thr
                            ),
                        ],
                        how="vertical",
                    ).sort("start_time")

                    df_query_missing = df_query_missing.filter(
                        pl.col("duration") > missing_duration_thr
                    )

                    if df_query_not_missing.select(pl.len()).item() == 0:
                        logging.warning(
                            f"Parameters 'missing_col_thr' or 'missing_duration_thr' are too stringent, can't find any eligible durations of time for file set {file_set_idx} of duration {df_query[f].select(pl.col('time').last() - pl.col('time').first()).collect().item()}."
                        )

                    df_query_missing = merge_adjacent_periods(
                        agg_df=df_query_missing, dt=data_loader.dt
                    )
                    df_query_not_missing = merge_adjacent_periods(
                        agg_df=df_query_not_missing, dt=data_loader.dt
                    )

                    df_query_missing = group_df_by_continuity(
                        df=df_query[f],
                        agg_df=df_query_missing,
                        missing_data_cols=missing_data_cols,
                    )
                    df_query_not_missing = group_df_by_continuity(
                        df=df_query[f],
                        agg_df=df_query_not_missing,
                        missing_data_cols=missing_data_cols,
                    )
                    df_query_missing = pl.concat(
                        [
                            df_query_missing,
                            df_query_not_missing.filter(
                                pl.col("duration") < minimum_not_missing_duration
                            ),
                        ],
                        how="vertical",
                    )
                    df_query_not_missing = df_query_not_missing.filter(
                        pl.col("duration") >= minimum_not_missing_duration
                    )

                    # .with_columns(continuity_group=(pl.arange(df_query_missing["continuity_group"].max() + 1, df_query_missing["continuity_group"].max() - 1 + df_query_not_missing.select(pl.len()).item())).cast(pl.Int32))

                    # filter out the continuity groups for which any measurement has 0 non-null values, can't impute then
                    df_query_not_missing = df_query_not_missing.select(
                        pl.col("duration"),
                        pl.col("start_time"),
                        pl.col("end_time"),
                        pl.col("continuity_group"),
                        cs.starts_with("is_missing")
                        & cs.matches(data_loader.turbine_signature),
                    )
                    ...
                    df_query_not_missing.with_columns(
                        continuity_group=pl.arange(0, pl.len()).cast(pl.Int32)
                    ).write_parquet(not_missing_fp)
                    df_query_missing.with_columns(
                        continuity_group=pl.arange(0, pl.len()).cast(pl.Int32)
                    ).write_parquet(missing_fp)
                    ...
                    logging.info("")
                    logging.info(
                        f"Starting to split by continuity group for {f}th of {len(file_set_indices)} file set."
                    )
                    logging.info(f"Initial continuity group index: {cg_init_idx}.")
                    logging.info(
                        f"Total missing duration = {df_query_missing['duration'].sum().total_seconds() / 3600} hours. Total not missing duration = {df_query_not_missing['duration'].sum().total_seconds() / 3600} hours."
                    )
                    # cg_init_idx = df_query[f-1].select(pl.col("continuity_group").max()).collect().item() + 1 if f > 0 else 0

                    df_query[f] = (
                        get_continuity_group_index(
                            continuity_groups_df=df_query_not_missing,
                            time_series_df=df_query[f],
                            cg_init_idx=cg_init_idx,
                        )
                    ...
                    logging.info(
                        f"Finished splitting by continuity group for {f}th of {len(file_set_indices)} file set."
                    )

                # df_query[f] = pl.scan_parquet(file_set_fp)
                ...
                if args.plot:
                    ...
                    for feature_type, marker in zip(missing_data_cols, ["o", "^"]):
                        for turbine_id, color in zip(
                            data_loader.turbine_ids,
                            colormaps["tab20c"](
                                np.linspace(0, 1, len(data_loader.turbine_ids))
                            ),
                        ):
                            df = (
                                df_query_missing.select(
                                    "duration",
                                    f"is_missing_{feature_type}_{turbine_id}",
                                )
                                .collect()
                                .to_pandas()
                            )
                            ax.scatter(
                                x=df["duration"].dt.seconds / 3600,
                                y=df[f"is_missing_{feature_type}_{turbine_id}"].astype(
                                    int
                                ),
                                marker=marker,
                                label=turbine_id,
                    ...
                    ax.set_title(
                        "Occurence of Missing Wind Speed (circle) and Wind Direction (triangle) Values vs. Missing Duration, for each Turbine"
                    )
                    ...
                    for feature_type, marker in zip(missing_data_cols, ["o", "^"]):
                        df = (
                            ...
                            .with_columns(
                                pl.sum_horizontal(
                                    [
                                        f"is_missing_{feature_type}_{tid}"
                                        for tid in data_loader.turbine_ids
                                    ]
                                ).alias(f"is_missing_{feature_type}")
                            )
                            .collect()
                            .to_pandas()
                        )
                    ...
                    ax.set_title(
                        "Occurence of Missing Wind Speed (circle) and Wind Direction (triangle) Values vs. Missing Duration, for all Turbines"
                    )
                    ax.set_xlabel("Duration of Missing Values [hrs]")
                    ax.set_ylabel("Number of Missing Values over this Duration")
                    h, l = ax.get_legend_handles_labels()
            ...
            if df_query.select(pl.len()).collect().item() == 0:
                logging.warning(
                    f"No remaining data rows after splicing time steps with over {missing_col_thr} missing columns"
                )
    ...
    if args.plot:
        ...
        plot_df = df_query.select(
            ["time", "continuity_group"]
            + [f"ws_horz_{tid}" for tid in ["wt005", "wt074", "wt075"]]
            + [f"ws_vert_{tid}" for tid in ["wt005", "wt074", "wt075"]]
        )
        #   .slice(0, int(3600*24))
        data_inspector.plot_time_series(
    ...
    if "impute_missing_data" in config["filters"]:
        ...
        if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
            logging.info(
                "Impute/interpolate turbine missing data from correlated measurements."
            )
            ...
            for feature in is_null.collect_schema().names():
                ...
                if (
                    feature.startswith("ws_horz")
                    or feature.startswith("ws_vert")
                    or feature.startswith("nd_cos")
                    or feature.startswith("nd_sin")
                ) and is_null.select(pl.col(feature)).item():
                    logging.error(
                        f"df contains {feature} column that are all null - will not be able to impute"
                    )
            ...
            logging.info(
                "Loading imputed/interpolated turbine missing data from correlated measurements."
            )
        ...
        assert all(
            typ == pl.Float32
            for typ in df_query.select(cs.float()).collect_schema().values()
        )
    else:
    ...
    if args.plot:
        ...
        data_inspector.plot_data_distribution(
            df_query,
            feature_types=["ws_horz", "ws_vert"],
            turbine_ids=data_loader.turbine_ids,
            distribution=norm,
        ...
        try:
            iec_curve = power_curve.IEC(
                windspeed_col="wind_speed", power_col="power_output", data=df_unpivoted
            )
            ...
            try:
                ...
                logging.warning(
                    "Sparse matrix error occurred, trying with fewer splines"
                )
    ...
    if "smooth" in config["filters"]:
        ...
        fp = config["processed_data_path"].replace(
            ".parquet", f"_smoothed_{smoothing_func}.parquet"
        )
        if args.reload_data or args.regenerate_filters or not os.path.exists(fp):
            ...
            smoothing_params = {
                k: v for k, v in config["filters"]["smooth"].items() if k != "function"
            }
            ...
            continuity_groups = (
                df_query.select("continuity_group")
                .unique()
                .collect()
                .to_series()
                .to_numpy()
            ...
            for cg_idx in range(len(continuity_groups)):
                cg_fp = os.path.join(
                    dirpath,
                    os.path.basename(config["processed_data_path"]).replace(
                        ".parquet", f"_cg{continuity_groups[cg_idx]}.parquet"
                    ),
                )
                if (
                    args.reload_data
                    or args.regenerate_filters
                    or not os.path.exists(cg_fp)
                ):
                    # for smoothing_func in ["butterworth", "moving_average", "savitzky_golay"]:
                    logging.info(
                        f"Smoothing continuity group {continuity_groups[cg_idx]} and writing to {cg_fp}."
                    )
                    ...
                    logging.info(
                        f"Loading existing smoothed continuity group {continuity_groups[cg_idx]} from {cg_fp}."
                    )
        ...
        if False:
            ...
            df_query.group_by("continuity_group", maintain_order=True).agg(
                pl.all().slice(0, pl.len() - 400)
            ).explode(pl.all().exclude("continuity_group")).collect().write_parquet(fp)
            ...
            for f, fp in enumerate(
                ...
                df_cg.slice(
                    0, df_cg.select(pl.len()).collect().item() - 400
                ).collect().write_parquet(fp)
    ...
    if "normalize" in config["filters"]:
        if (
            ...
            or not all(
                ...
                for fp in [
                    config["processed_data_path"].replace(
                        ".parquet", f"_{ll}_normalized.parquet"
                    )
                    for ll in ["unsmoothed", "smoothed"]
                ]
            )
        ):
            ...
            for l, ll in zip(dataset_labels, ["unsmoothed", "smoothed"]):
                ...
                if not os.path.exists(fp):
                    logging.error(f"{l} dataset does not exist at {fp}!")
                    continue
                ...
                cols = [
                    c
                    for c in dfq.collect_schema().names()
                    if any(c.startswith(ft) for ft in feature_types)
                ]
                ...
                for col in cols:
                    ...
                    norm_vals[f"{col}_mean"] = (
                        dfq.select(pl.col(col).mean()).collect().item()
                    )
                    norm_vals[f"{col}_std"] = (
                        dfq.select(pl.col(col).std()).collect().item()
                    )

                norm_vals = pl.DataFrame(norm_vals).select(pl.all())
                norm_vals.write_csv(
                    config["processed_data_path"].replace(
                        ".parquet", f"_{ll}_normalization_consts.csv"
                    )
                )
                ...
                dfq = dfq.select(
                    ...
                    + [
                        (
                            ...
                            / norm_vals.select(f"{col}_std").item()
                        ).name.keep()
                        for col in cols
                    ]
                ).with_columns(cs.float().cast(pl.Float32))
                ...
                for col in cols:
                    logging.info(
                        f"Feature {col} mean = {dfq.select(pl.col(col).mean()).collect().item()}"
                    )
                    logging.info(
                        f"Feature {col} std = {dfq.select(pl.col(col).std()).collect().item()}"
                    )

                assert dfq.select("time").collect().to_series().is_sorted()
                assert all(
                    typ == pl.Float32
                    for typ in dfq.select(cs.float()).collect_schema().values()
                )

                logging.info(f"Started sinking {ll} dataframe.")
                dfq.collect().write_parquet(
                    config["processed_data_path"].replace(
                        ".parquet", f"_{ll}_normalized.parquet"
                    )
                )
            ...
            if "smooth" in config["filters"]:
                df_query = pl.scan_parquet(
                    config["processed_data_path"].replace(
                        ".parquet", "_smoothed_normalized.parquet"
                    )
                )
            else:
                df_query = pl.scan_parquet(
                    config["processed_data_path"].replace(
                        ".parquet", "_unsmoothed_normalized.parquet"
                    )
                )
            ...
            if False:
                ...
                ).group_by("continuity_group", maintain_order=True).agg(
                    pl.all().slice(0, pl.len() - 400)
                ).explode(pl.all().exclude("continuity_group")).collect().write_parquet(
                    config["processed_data_path"].replace(
                        ".parquet", "_smoothed_normalized.parquet"
                    )
                )
        ...
        if args.plot:
            ...
            plot_df = df_query.select(
                ...
                + [
                    ...
                    for tid in ["wt005", "wt074", "wt075"]
                ]
            )
            #   .slice(0, int(3600*24))

            continuity_groups = np.random.choice(
                continuity_groups, size=min(5, len(continuity_groups)), replace=False
            )
            data_inspector.plot_time_series(
                # pl.concat([df.slice(0, ROW_LIMIT) for df in df_query.collect().partition_by("continuity_group")], how="vertical").lazy(),
                plot_df,
